...
__global__ void render_shared(cv::cuda::PtrStepSz<vec3_t> img, cv::cuda::PtrStepSz<vec3_t> hdr, int max_x, int max_y, camera_t* cam_o, sphere_t* ls, int count, disk_t* disk_s) {
    ...
    if (threadIdx.x == 0 && threadIdx.y == 0) {
        ...
        blackhole[0] = sphere_t{ { 0,0,0 }, 0.2f, { 0,0,0 } , 0.0025f };
        disk[0] = *disk_s;
    ...
    //ray r = ray(cam->origin, cam->lower_left_corner + (u * cam->horizontal) + (v * cam->vertical) - cam->origin, i, j, hdr);
    ray_t r1 = ray_t{ cam->origin, cam->lower_left_corner + (u * cam->horizontal) + (v * cam->vertical) - cam->origin , 0.02f, 256 };
...
__global__ void render_constant(cv::cuda::PtrStepSz<vec3_t> img, cv::cuda::PtrStepSz<vec3_t> hdr, int max_x, int max_y, int count) {
    ...
    //ray r = ray(cam->origin, cam->lower_left_corner + (u * cam->horizontal) + (v * cam->vertical) - cam->origin, i, j, hdr);
    ray_t r1 = ray_t{ cam_const->origin, cam_const->lower_left_corner + (u * cam_const->horizontal) + (v * cam_const->vertical) - cam_const->origin , 0.02f, 256 };
    //vec3_t col = r.march(ls, blackhole, count, disk);
...
sphere_t* createSceneStruct(float angle) {
    ...
    scene[0] = sphere_t{ vec3_t{ -.8f * cosf(angle) , .8f * sinf(angle), 0 }, 0.1f, { 1,1,.8f }, 0 };
    scene[1] = sphere_t{vec3_t{ 0.95f,0,0 }, 0.05f, {.9f ,1, 1 }, 0};
...
cv::Mat renderScene(int img_w, int img_h, camera_t* cam, float& angle, Mat3f& hdr, sphere_t* scene, disk_t* disk) {
    ...
    dim3 grid_size(img_w / 16, img_h / 16);
...
cv::Mat renderSceneConst(int img_w, int img_h, camera_t* cam, float& angle, Mat3f& hdr) {
    ...
    //cv::resize(hdr, hdr, Size(img_w, img_h));
    cudaMemcpyToSymbol(cam_const, cam, sizeof(camera_t), 0);
    ...
    render_constant <<< grid_size, block_size >> > (gpu_img, gpu_hdr, img_w, img_h, 2);
    //printf("%s \n", cudaGetErrorString(cudaGetLastError()));
