...
__device__ vec3_t march(ray_t* r, cv::cuda::PtrStepSz<vec3_t> hdr, sphere_t* obj_ls, sphere_t* blackhole, int count, disk_t* disk) {
	...
	for (int i = 0; i < N_STEPS; i++) {
		...
		r->dir = norm(r->dir + 3.0f * 0.00005f * 0.5f * r_g / (d * d * d));
	...
	if(color.x<0)
		color = hdr(.5f * ((asinf(r->dir.y) / PI) + 1) * 512, (1 - (atan2f(r->dir.z, r->dir.x)) / (2 * PI) + 0.5f) * 1024);
	return color;
}
...
__device__ vec3_t march(ray_t& r, cv::cuda::PtrStepSz<vec3_t> hdr, sphere_t* obj_ls, sphere_t* blackhole, int count, disk_t* disk) {
	...
	for (int i = 0; i < N_STEPS; i++) {
		...
		r.dir = norm(r.dir + 3.0f * 0.5f * 0.00005f * r_g / (d * d * d));
	}
	color = hdr(.5f * ((asinf(r.dir.y)  / PI)+1) * 512, (1 - (atan2f(r.dir.z, r.dir.x)) / (2 * PI) +0.5f) * 1024);
	return color;
}
